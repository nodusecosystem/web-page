...
export default function GlowCursorFx({
  containerRef,
  canvasRef,
  maxDevicePixelRatio,
  opacity = 1,
  ...props
}: Readonly<GlowCursorFxProps>) {
  const propsRef = useRef<GlowCursorFxConfig>({ ...props, maxDevicePixelRatio, opacity })

  useEffect(() => {
    propsRef.current = { ...props, maxDevicePixelRatio, opacity }
  })
