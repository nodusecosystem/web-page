...
export type DepthCarouselItem =
  | string
  | { image: string; alt?: string; content?: undefined }
  | { content: ReactNode; alt?: string; image?: undefined };

type NormalizedItem = { image?: string; alt: string; content?: ReactNode };
...
const normalizeItem = (it: DepthCarouselItem): NormalizedItem => {
  if (typeof it === 'string') return { image: it, alt: '', content: undefined };
  if ('image' in it) return { image: it.image, alt: it.alt ?? '', content: undefined };
  return { image: undefined, alt: it.alt ?? '', content: it.content };
};
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="absolute left-1/2 top-1/2 cursor-pointer overflow-hidden border border-teal-light/25 bg-[#000F13] shadow-[0_30px_60px_-20px_rgb(7_25_25_/_0.65),0_8px_20px_-10px_rgb(7_25_25_/_0.5)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]"
            ref={el => {
          ...
          >
            {item.image ? (
              ...
              <div className="h-full w-full overflow-hidden rounded-[inherit]">{item.content}</div>
            )}
            {item.image ? (
              <span
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="absolute left-4 top-1/2 z-[3000] grid h-[42px] w-[42px] -translate-y-1/2 cursor-pointer place-items-center rounded-full border border-teal-light/30 bg-[rgb(7_25_25/0.6)] text-teal-light backdrop-blur-md transition-[background,border-color,transform] duration-200 hover:border-teal-light/60 hover:bg-[rgb(7_25_25/0.85)] active:scale-95"
          ...
          <button
            type="button"
            className="absolute right-4 top-1/2 z-[3000] grid h-[42px] w-[42px] -translate-y-1/2 cursor-pointer place-items-center rounded-full border border-teal-light/30 bg-[rgb(7_25_25/0.6)] text-teal-light backdrop-blur-md transition-[background,border-color,transform] duration-200 hover:border-teal-light/60 hover:bg-[rgb(7_25_25/0.85)] active:scale-95"
      ...
      {showIndicators && count > 1 && (
        <div
          className="absolute bottom-4 left-1/2 z-[3000] flex -translate-x-1/2 gap-2 rounded-full bg-[rgb(7_25_25/0.5)] px-3 py-2 backdrop-blur-sm"
          role="tablist"
          aria-label="Slides"
        >
          {data.map((_, i) => (
